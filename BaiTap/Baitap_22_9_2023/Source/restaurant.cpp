...
int8_t Table::searchTableDish(uint16_t id){
    ...
    do{
        DishOnTable table_dish_place=seeDish(place_dish-1);
        if(table_dish_place.getID()==id) return (place_dish-1);
        else{
            table_dish_place=seeDish(place_dish);
            if(table_dish_place.getID()==id) return (place_dish);
            else{
                table_dish_place=seeDish(place_dish+1);
                if(table_dish_place.getID()==id) return (place_dish+1);
                else{
...
int16_t dishPlace(uint16_t id){
    ...
    do{
        ...
        advance(dish_place,placeDish-1);
